...
/** True when `.pipeline` exists at `start` or any ancestor — i.e. the cwd
...
export async function runUi(args: string[]): Promise<number> {
  ...
  if (opts.json) {
    ...
    if (!inPipelineProject) {
      process.stdout.write(
        '  (this project has no .pipeline yet — nothing will appear until you run /pipeline:design or /pipeline:run)\n',
      );
    }
  }
