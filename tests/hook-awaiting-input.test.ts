/**
 * Notification → `run.awaiting_input` — src/hooks/analytics-relay.ts (design 05).
 *
...
import { classifyNotification, handleNotification } from "../src/hooks/analytics-relay.ts";
...
function spawnHookVerbose(
  ...
  const r = spawnSync(process.execPath, hookArgv("analytics-relay"), {
    cwd: root,
