/**
 * Stop/SubagentStop stats relay — src/hooks/stats-relay.ts.
 *
 ...
 *     algorithm (tests/fixtures/stats-relay.pre-refactor.ts) and the real
...
import { hookArgv } from './_hook-entry';

/** The real relay, spawned the way the plugin spawns it: `pipeline hook
 *  stats-relay`. */
const HOOK_ARGV = hookArgv('stats-relay');
/** The frozen pre-refactor algorithm, still a plain `bun <script>` — see that
 *  fixture's own header for why it deliberately did not follow the relay into
 *  subcommand shape. */
const PRE_REFACTOR_ARGV = [resolve(import.meta.dir, 'fixtures/stats-relay.pre-refactor.ts')];
...
function spawnRelay(argv: string[], cwd: string, payload: Record<string, unknown>): HookRun {
...
describe('stats-relay byte-equivalence (pre-refactor vs refactored)', () => {
  test('SubagentStop payload → identical runs.jsonl from both the frozen pre-refactor script and the real hook', () => {
    ...
    const oldRun = spawnRelay(PRE_REFACTOR_ARGV, rootOld, payloadFor(rootOld));
    const newRun = spawnRelay(HOOK_ARGV, rootNew, payloadFor(rootNew));
...
describe('stats-relay end-to-end (Stop/SubagentStop, real hook)', () => {
  test('SubagentStop: enriches a tokens:null record from the payload transcript', () => {
    ...
    const r = spawnRelay(HOOK_ARGV, root, {
      session_id: 'sess-1',
      hook_event_name: 'SubagentStop',
      transcript_path: join(root, 'transcript-home', `${runId}.jsonl`),
      cwd: root,
  ...
  test('disabled gate (PIPELINE_STATS_ENABLED=0) → no-op, record stays null', () => {
    ...
    const r = spawnSync(process.execPath, HOOK_ARGV, {
      cwd: root,
      env,
  ...
  test('missing transcript_path → no-op, no throw', () => {
    ...
    const root = buildFixtureTemplate(runId);
    const r = spawnRelay(HOOK_ARGV, root, { session_id: 'sess-1', cwd: root });
    expect(r.status).toBe(0);
    const rec = JSON.parse(readFileSync(runsJsonlPath(root), 'utf8').trim());
    expect(rec.tokens).toBeNull();
  });
