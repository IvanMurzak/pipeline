 ...
 *
 * A verbatim copy of `src/hooks/stats-relay.ts` as it existed BEFORE the a1
 ...
 * `hook-stats-relay.test.ts` out of `tests/` and into the
 * plugin's own root `tests/` — the depth changed, the copy did not. And a
 * THIRD time in `p6`, following the same test into the CLI's own repository.
 *
 * It also stays a STANDALONE SCRIPT, `bun`-spawnable, while the real relay it
 * characterizes became `pipeline hook stats-relay`. That asymmetry is the
 * point: a frozen fixture must not be re-plumbed every time the thing it is
 * frozen against changes shape, or it stops being a fixed reference.
 *
 ...
 * `src/lib/transcript-walk` (which `k2` promoted out of `vendor/` to ordinary
 * source), holding exactly those functions that the real hook already folds
 * through — it is now their only definition. An import path is not
